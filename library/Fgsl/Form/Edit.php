		...
		foreach($this->_options[self::DATA] as $fieldName => $fieldValue)
		{
			...
			try {
				...
				if ($typeElement == Fgsl_Form_Constants::SELECT)
				{
					...
					//Não irá validar o conteúdo que está sendo passado para gravar no banco
					//pois quando o SELECT é populado por AJAX é gerado um erro.
					$text->setRegisterInArrayValidator(false);
				}
